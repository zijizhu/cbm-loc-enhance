...
class IdentityTransform:
  ...
  def __call__(self, x):
    return [
      torch.tensor(
        self.label_remap.get(x[0].cpu().detach().item() - 1, self.num_classes),
        dtype=torch.long,
      ),
...
def generate_data(root_dir, resol, transform, config=CELEBA_CONFIG, seed=42, output_dataset_vars=False):
  ...
  if use_binary_vector_class:
    ...
    if config.get("num_hidden_concepts", 0):
      ...
      hidden_concepts = sorted(sorted_concepts[num_concepts : min((num_concepts + num_hidden), len(sorted_concepts))])
    else:
      hidden_concepts = []
    ...
    sorted_labels = list(map(lambda x: x[0], sorted(zip(vals, counts), key=lambda x: -x[1])))
    ...
    train_idxs = np.where(list(map(lambda x: x.cpu().detach().item() - 1 in label_remap, celeba_train_data.identity)))[0]
    celeba_train_data = torch.utils.data.Subset(
      celeba_train_data,
      train_idxs,
    )
...
if __name__ == "__main__":
  train_dl, val_dl, test_dl, imbalance, (num_concepts, len_label_remap, concept_group_map) = generate_data(
    root_dir=os.path.expanduser("~/Developer/Research/datasets/"), resol=224, output_dataset_vars=True
  )
