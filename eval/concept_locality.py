...
def get_activation_maps(ppnet, test_loader, corre_proto_num=10):
  ...
  corre_proto_num = 5
  a_weight = ppnet.attributes_predictor.detach()  # (112, 2000)
...
@torch.no_grad()
def evaluate_concept_locality(all_activation_maps, all_img_ids, bbox_half_size=45, img_size=224):
  """
  ...
  for attri_idx in tqdm(range(n_attributes)):
    ...
    bhz = bbox_half_size
    pred_y1, pred_y2, pred_x1, pred_x2 = (
      torch.where(mi_h - bhz >= 0, mi_h - bhz, 0),
    ...
    part_locs = torch.gather(
      part_locs, 1, part_indexes
    )  # (n_select_samples, Np, 2), Np is the number of part location annotations of current attribute in the image
    ...
    cal_pred_bboxes, cal_part_locs, _ = (
      pred_bboxes[cal_img_indexes],
      part_locs[cal_img_indexes],
      part_exist[cal_img_indexes],
    )  # (n_cal_samples, 4), (n_cal_samples, Np, 2), (n_cal_samples, Np)

    cal_pred_bboxes = cal_pred_bboxes[:, None]  # (n_cal_samples, 1, 4)
    cal_cond1, cal_cond2, cal_cond3, cal_cond4 = (
      cal_part_locs[:, :, 0] - cal_pred_bboxes[:, :, 0] >= 0,
      cal_part_locs[:, :, 0] - cal_pred_bboxes[:, :, 1] <= 0,
      cal_part_locs[:, :, 1] - cal_pred_bboxes[:, :, 2] >= 0,
      cal_part_locs[:, :, 1] - cal_pred_bboxes[:, :, 3] <= 0,
    )  # Each one: (n_cal_samples, Np)
    cal_part_inside = (
      torch.stack([cal_cond1, cal_cond2, cal_cond3, cal_cond4], dim=2).sum(dim=-1) == 4
    )  # (n_cal_samples, Np), estimate whether the ground-truth part location is inside the prediction bbox
    cal_part_inside = cal_part_inside.sum(dim=-1) > 0  # (n_cal_samples,)

    loc_acc = (
      cal_part_inside.sum(dim=0) / cal_part_inside.shape[0]
    )  # Calculate the ratio of images that the ground-truth part location is inside the prediction bbox
